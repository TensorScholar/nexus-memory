use nexus_memory::numa::{NumaAllocator, AllocationPolicy, NodeId};
use nexus_validation::numa_verify::{detect_numa_topology, verify_physical_location, verify_buffer_placement};
...
fn verify_numa_physical_placement() {
    ...
    if !topology.is_numa || topology.nodes.len() < 2 {
        println!("SKIP: System has insufficient NUMA nodes (found: {})", topology.nodes.len());
    ...
    println!("- Allocating 10MB buffer explicitly on Node {}...", target_node);
    let allocator = NumaAllocator::new();
    let size = 10 * 1024 * 1024; // 10MB
    let count = size / std::mem::size_of::<u64>();
    
    // Use OnNode policy
    let ptr = allocator.allocate::<u64>(count, AllocationPolicy::OnNode(NodeId(target_node)))
    ...
    println!("  - Pages Validated: {}/{}", report.pages_correct, report.pages_checked);
    ...
    if confirmed && report.verified {
        println!("> [PASS] Physical placement verified. Memory is resident on Node {}.", target_node);
    } else {
